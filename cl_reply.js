...
let clreplycount = 45
let minwaittime = process.env.MINWAITTIME ? process.env.MINWAITTIME : 250
let maxwaittime = process.env.MAXWAITTIME ? process.env.MAXWAITTIME : 750
            ...
            if (isrun) {
                ...
                for (var j = 0; j < replycount; j++) {
                    ...
                    if (!y || y < 100) {//等待时间为空或者小于100秒
                        console.log('程序可能出问题了，停止运行')
                        await tgBotNotify($.name, '程序可能出问题了，停止运行\n今日发帖数量：' + jrft, `\n`);
                        break;
                    }
